...
[[ stitchable ]] half4 checkerboard(float2 position, half4 color, half4 replacement, float size, float width) {
    ...
    // is odd.
    float isColor = (posInChecks.x ^ posInChecks.y) & 1;

    // If it's odd send back the replacement color,
    // otherwise send back the original
    return color.a * mix(isColor * half4(1, 1, 1, 1), replacement, saturate(position.x / width));
}
